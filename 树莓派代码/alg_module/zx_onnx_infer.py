...
detect_model = os.path.join(root_dir, "weights/plate_detect.onnx")
rec_model = os.path.join(root_dir, "weights/plate_rec_color.onnx") 
...
def license_plate_det(img_bgr):
    img,r,left,top = detect_pre_precessing(img_bgr,img_size) #检测前处理
    ...
    result_list = rec_plate(outputs,img_bgr,session_rec)
    return result_list


def decodePlate(preds, scores):        #识别后处理
    pre=0
    newPreds=[]
    score_list = []
    for i in range(len(preds)):
        if preds[i]!=0 and preds[i]!=pre:
            ...
            score_list.append(scores[i][preds[i]])
        pre=preds[i]
    plate=""
    for i in newPreds:
        plate+=plateName[int(i)]
    return plate, np.array(score_list).mean(), np.array(score_list).min()
...
def get_plate_result(img,session_rec): #识别后处理
    ...
    x_exp = np.exp(y_onnx_plate - np.max(y_onnx_plate, axis=-1, keepdims=True))
    scores = x_exp / np.sum(x_exp, axis=-1, keepdims=True)
    #scores = scores[index]

    index_color = np.argmax(y_onnx_color)
    plate_color = plate_color_list[index_color]
    # print(y_onnx[0])
    plate_no, char_score_mean,  char_score_min = decodePlate(index[0], scores[0])
    return plate_no,plate_color,char_score_mean,  char_score_min
...
def rec_plate(outputs,img0,session_rec):  #识别车牌
    ...
    for output in outputs:
        ...
        plate_no,plate_color,char_score_mean,char_score_min = get_plate_result(roi_img,session_rec)
        ...
        result_dict['char_score_mean']=char_score_mean
        result_dict['char_score_min']=char_score_min
        x_center = (rect[0] + rect[2]) / 2.0
        if x_center <= (img0.shape[1] / 2.0):
            result_dict['in_out'] = 'in'
        else:
            result_dict['in_out'] = 'out'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--image_path', type=str, default='alg_module/imgs', help='source') 
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--output', type=str, default='alg_module/result1', help='source') 
    ...
    for pic_ in file_list:
        ...
        for result in result_list:
            print(f"img={pic_} license={result['plate_no']}")
